...
for f= 0:columns-1;
    for n = 80*f+1 : 80*(f+1)
        for m= 1: floor(Refined_p(f+1))-1 
            
            if f == columns-1
               Am_syn_voiced(n) = real(Refined_p_Am(m,f+1));
               w0_syn_voiced(n) = 2*pi/Refined_p(f+1);
            else               
               Am_syn_voiced(n) =  (f+1)-(n/Tskip) * real(Refined_p_Am(m,f+1)) + (n/Tskip-f)* real(Refined_p_Am(m,f+2));
               w0_syn_voiced(n) =  (f+1)-(n/Tskip) * 2*pi/Refined_p(f+1) + (n/Tskip-f)* 2*pi/Refined_p(f+2);
